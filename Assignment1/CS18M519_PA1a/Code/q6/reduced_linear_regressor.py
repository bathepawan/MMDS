...
class ReducedLinearRegressor(object):
    def __init__(self, dataset, features, output, number_of_runs=5, test_size=0.2):
    ...
    def compute_rse(self):
        ...
        for split in range(1, self.number_of_runs+1):
            x_train, x_test, y_train, y_test = train_test_split(
                self.dataset.loc[:, self.features],
                self.dataset.loc[:, self.output],
                test_size=self.test_size, random_state=split)
            training_data = x_train.copy()
            training_data[self.output] = y_train
            test_data = x_test.copy()
            test_data[self.output] = y_test
            ...
            linear_model.fit(training_data.loc[:, self.features],
                             training_data.loc[:, self.output])
            # print('Coefficients for split {} are {}'.format(split, linear_model.coef_))
            y_predicted = linear_model.predict(test_data.loc[:, self.features])
        ...
        self.print_model(coefs=self.reg_coefficients[self.rss.index(min(self.rss))],
                         names=cnc_headers[5:-1], output=self.output)
